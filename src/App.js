...
class App extends Component {
  ...
  fileUploadHandler = () => {
    ...
    for (var x = 0; x < this.state.selectedFile.length; x++) {
      data.append(
        "file",
        this.state.selectedFile[x],
        this.state.selectedFile[x].name
      );
    }
    axios
      .post("http://localhost:5000/api/v1.0/anonymization", data, {
    ...
    axios.get("http://localhost:5000/api/v1.0/publish");
    var es = new EventSource("http://localhost:5000/api/v1.0/stream");
    es.addEventListener(
      "myevent",
      function (e) {
        // 'myevent' 이벤트의 데이터 처리
